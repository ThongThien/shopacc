...
export default function RegisterForm() {
  ...
  function pwStrength(pw: string) {
    if (pw.length < 6) return { label: "Yếu", color: "var(--color-danger)", w: "25%" };
    if (pw.length < 8) return { label: "Trung bình", color: "var(--color-warning)", w: "50%" };
    if (/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(pw)) return { label: "Mạnh", color: "var(--color-price)", w: "100%" };
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!username.trim()) { notify("error", "Vui lòng nhập tên đăng nhập"); return; }
    if (username.trim().length < 3) { notify("error", "Tên đăng nhập tối thiểu 3 ký tự"); return; }
    if (!email.trim()) { notify("error", "Vui lòng nhập email"); return; }
    if (!/\S+@\S+\.\S+/.test(email)) { notify("error", "Email không hợp lệ"); return; }
    if (!password) { notify("error", "Vui lòng nhập mật khẩu"); return; }
    if (password.length < 6) { notify("error", "Mật khẩu tối thiểu 6 ký tự"); return; }
    ...
    <>
      <form onSubmit={handleSubmit} className="auth-card card">
        <div style={{ textAlign: "center", marginBottom: 4 }}>
          ...
          <p style={{ margin: 0, color: "var(--color-text-muted)", fontSize: 14 }}>Tạo tài khoản để bắt đầu</p>
        </div>

        <div>
          <label style={{ display: "block", marginBottom: 6, fontSize: 13, fontWeight: 600, color: "var(--color-text-secondary)" }}>Tên đăng nhập</label>
          <input className="input" placeholder="Tối thiểu 3 ký tự" value={username}
          ...
          {strength && (
            <div style={{ marginTop: 6 }}>
              <div style={{ height: 4, borderRadius: 2, background: "var(--color-bg-hover)" }}>
                <div style={{ height: "100%", width: strength.w, background: strength.color, borderRadius: 2, transition: "width 0.2s" }} />
              </div>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--color-text-muted)", cursor: "pointer" }}>
          ...
          Tôi đồng ý với{" "}
          <button type="button" onClick={() => setTermsOpen("terms")} style={{ color: "var(--color-primary)", background: "none", border: "none", cursor: "pointer", padding: 0, fontWeight: 600, fontSize: 13 }}>Điều khoản</button>
          {" "}và{" "}
          <button type="button" onClick={() => setTermsOpen("privacy")} style={{ color: "var(--color-primary)", background: "none", border: "none", cursor: "pointer", padding: 0, fontWeight: 600, fontSize: 13 }}>Chính sách bảo mật</button>
        </label>
        ...
        <p style={{ textAlign: "center", fontSize: 14, color: "var(--color-text-muted)", margin: 0 }}>
          Đã có tài khoản? <Link href="/login" style={{ color: "var(--color-primary)", fontWeight: 600 }}>Đăng nhập</Link>
        </p>
      </form>
      ...
      {termsOpen && (
        <div className="modal-overlay" onClick={() => setTermsOpen(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <h2>{termsOpen === "terms" ? "Điều khoản sử dụng" : "Chính sách bảo mật"}</h2>
            {termsOpen === "terms" ? (
              <div style={{ color: "var(--color-text-secondary)", fontSize: 14, lineHeight: 1.7 }}>
                ...
                <ul style={{ paddingLeft: 20 }}>
                  ...
                  <li>Shopacc có quyền khóa tài khoản nếu phát hiện vi phạm.</li>
                  <li>Mọi giao dịch nạp tiền là cuối cùng và không hoàn lại.</li>
                  <li>Sau khi mua acc, bạn có trách nhiệm đổi mật khẩu ngay.</li>
                </ul>
              </div>
            ) : (
              <div style={{ color: "var(--color-text-secondary)", fontSize: 14, lineHeight: 1.7 }}>
                ...
                <ul style={{ paddingLeft: 20 }}>
                  ...
                  <li>Mật khẩu được mã hóa một chiều (BCrypt), không ai đọc được.</li>
                  <li>Thông tin tài khoản game được mã hóa AES-256.</li>
                  <li>Bạn có thể yêu cầu xóa tài khoản bất kỳ lúc nào.</li>
                </ul>
              </div>
            )}
            <div className="modal-actions">
              <button className="btn-primary" type="button" onClick={() => setTermsOpen(null)}>Đã hiểu</button>
            </div>
          </div>
        </div>
      )}
